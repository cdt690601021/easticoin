...
const char * const BITCOIN_CONF_FILENAME = "easticoin.conf";
const char * const BITCOIN_PID_FILENAME = "easticoin.pid";
...
#else
    const char* pszModule = "easticoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Easticoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Easticoin";
#else
    // Unix
    return pathRet / ".easticoin";
#endif
#endif
}
